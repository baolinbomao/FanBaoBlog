    ...
    req.onreadystatechange = function () {
        if(req.readyState === 4){
            if(req.responseText === 'ok'){
                var likeit = documemt.getElementById('like');
                likeit.innerText = parseInt(likeit.innerText) + 1 + '';
            }else{
                console.log(req.responsText + '出错了！');
            }
        }
    }
    req.send()
...
function showReplyDialog(host_name, id, host_id) {
    ...
    replydialog.getElementsByClassName.display = 'block'
